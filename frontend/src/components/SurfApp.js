...
export default class SurfApp extends React.Component{
	...
	async getActiveStations(){
		...
		const result = await fetch("/station-api/stations/", {headers: json_header} );
	...
	parseActiveStations(stations){
		...
		for(let i = 0; i < stationsLen ; i++){
				station_list.push({ value: stations[i].station_id,  label: stations[i].name, coordinates: [stations[i].lat, stations[i].lon]});	
		}
		return station_list;
	}
