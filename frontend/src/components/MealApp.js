...
function MealRecipe(props){
    ...
    return(
        <div className="meal-recipe">
            <button type="button" className="back" onClick={props.clickFunction}>Back</button>
            <h2>{recipeName}</h2>
...
export default class MealApp extends React.Component{
    ...
    mealClick = (mealId) =>{
        ...
        this.setState({ back: this.state.meals, meals: null});
    ...
    goBack = () =>{
        this.setState({recipe: null, meals: this.state.back });
    }
        ...
        return(
            ...
            }
            {
                choiceMade &&   <div className="meal-display">
                                    ...
                                    { !recipe && meals && <MealChoices meals={meals} clickFunction={this.mealClick} />}
                                    { recipe && <MealRecipe recipe={recipe[0]} clickFunction={this.goBack}/>}
                                </div>
            }
            </div>
        );
